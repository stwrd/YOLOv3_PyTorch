...
def test(config):
    ...
    # Start inference
    batch_size = 1
    out_dir = '/media/hzh/ssd_disk/smoke_and_call/test_head'
            ...
            if boxes is not None:
                ...
                for i, box in enumerate(boxes):
                    ...
                    if box_w >= 1 and box_h >= 1:
                        ...
                        x1, y1, x2, y2 = max(int(center_x - l * 5 // 8), 0), max(int(center_y - l // 2), 0), min(
                            int(center_x + l * 5 // 8), img.shape[1]), min(int(center_y + l * 3 // 4), img.shape[0])
                        roi_img = img[y1:y2, x1:x2]
                        img_basename = os.path.basename(images_path[step])
                        nosuffix_name = os.path.splitext(img_basename)[0]
                        results_img_path = os.path.join(out_dir, '{}_{:0=2}.jpg'.format(nosuffix_name,i))
