...
def test(config):
    ...
    net.load_darknet_weights('../weights/yolov3_head_final.weights')
    ...
    # prepare images path
    tar_path = r'E:\workspace\data\data_smoke\output\src\pos'
    dst_path = r'E:\workspace\data\data_smoke\output\src\detect'
    images_name = os.listdir(tar_path)
    images_path = [os.path.join(tar_path, name) for name in images_name if name.endswith('.jpg') or name.endswith('.jpeg')]
        ...
        for idx, boxes in enumerate(batch_detections):
            ...
            if boxes is not None:
                for i, box in enumerate(boxes):
                    ...
                    #add 5 pixel
                    x1, y1, x2, y2 = max(int(x1-box_w/4),0), max(int(y1),0), min(int(x2+box_w/4),img.shape[1]), min(y2+box_h/4, img.shape[0])
                    roi_img = img[int(y1):int(y2),int(x1):int(x2)]
                    img_basename = os.path.basename(images_path[step])
                    nosuffix_name = os.path.splitext(img_basename)[0]
                    results_img_path = os.path.join(dst_path, '{}_{:0=2}.jpg'.format(nosuffix_name,i))
                    cv2.imwrite(results_img_path,roi_img)
